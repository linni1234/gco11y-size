...
func Operations(ctx gocommon.FileContext) []model.Operation {
	if !gocommon.HasImport(ctx.File, "github.com/gofiber/fiber/v2") && !gocommon.HasImport(ctx.File, "github.com/gofiber/fiber") {
		return nil
	}
	receivers := gocommon.ImportReceivers(ctx.File, "github.com/gofiber/fiber/v2", "github.com/gofiber/fiber")
	...
	gocommon.ForEachCall(ctx.File, func(call *ast.CallExpr) {
		...
		operations = append(operations, gocommon.Operation(ctx.ServiceName, "SERVER", "http", method, route, handler, ctx.SourcePath, "fiber", "high"))
	})
	return operations
}
